...
// GuildActivity represents a user's activity within a particular guild.
...
// GetGuildActivity returns the guild activity using the user and guild ID strings.
...
// GetGuildActivityByID returns the guild activity using the UUID primary key.
func GetGuildActivityByID(id uuid.UUID) (*GuildActivity, error) {
	m := &GuildActivity{}
	return m, DB.First(m, "id = ?", id).Error
}

// GetGuildActivities returns the activity records for a particular guild, ranked by XP.
...
// CreateGuildActivity creates a GuildActivity record in the database.
func CreateGuildActivity(activity *GuildActivity) error {
	activity.ID = guildActivityID(activity.UserID, activity.GuildID)
	return DB.Create(activity).Error
}

// guildActivityID returns the ID of a GuildActivity record based on the user and guild IDs.
func guildActivityID(userID, guildID string) uuid.UUID {
	return uuid.NewV5(namespace, fmt.Sprintf(userID+guildID))
}

// IncrementMessagesSent increments the number of messages sent by the user within the given guild.
...
// IncrementVoiceChannelDuration increments the duration spent within voice channels by the user within the given guild.
